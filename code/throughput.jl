...
function getThroughputGraphs(
    ...
    for t = 1:length(topologies)
        ...
        links, capacity, link_probs, nodes =
            readTopology(topology, downscale = demand_downscales[t])
        scenarios_all_top = []
        scenario_probs_top = []
        for i = 1:iterations
    ...
    for alg = 1:length(algorithmns)
            ...
            for d = 1:num_demands
                ...
                for i = 1:iterations
                    if algorithmns[alg] == "TEAVAR"
                        if teavar_paths != "KSP"
                            T, Tf, k = parsePaths(
                                "$(topologies[t])/paths/$(teavar_paths)",
                                links,
                                flows,
                            )
                        else
        ...
        beta_avg_vars =
            beta_var_totals ./
            (num_demands * length(topologies) * iterations * length(x_vals))
        beta_avg_cvars =
            beta_cvar_totals ./
            (num_demands * length(topologies) * iterations * length(x_vals))
    ...
    if plot
        ...
        for bar = 1:nbars
            ...
            PyPlot.bar(
                collect(1:ngroups) .- 1 .+ (barWidth .* bar),
                group_ys,
                alpha = 0.8,
                width = barWidth,
            )
