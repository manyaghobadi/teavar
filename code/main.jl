...
Base.@ccallable function julia_main()::String
    ...
    experiment =
        parsed_args["experiment"] != nothing ? parsed_args["experiment"] :
        readInput("Experiment (scenario_coverage): ", "scenario_coverage", String)
    ...
    if experiment == "scenario_coverage"
        ...
        cutoffs = map(
            ...
            split(
                readInput(
                    "Cutoffs (.001,.0001,.00001,.000001,.0000001): ",
    ...
    elseif experiment == "optimizer_times"
        topologies = split(
            readInput("Topologies (B4,IBM,XNet,ATT): ", "B4,IBM,XNet,ATT", String),
            ",",
        )
        cutoffs = map(
            i -> parse(Float64, i),
    ...
    elseif experiment == "throughput_guarantee"
        ...
        availabilities = map(
            ...
            split(
                readInput(
                    "Availabilities (.99, .999, .9999, .99995, .99999): ",
                    ".99, .999, .9999, .99995, .99999",
                    String,
                ),
                ",",
            ),
    ...
    elseif experiment == "availability"
        ...
        algorithms = split(
            readInput(
                "Algorithms (TEAVAR,SMORE,ECMP,FFC-1): ",
                "TEAVAR,SMORE,ECMP,FFC-1",
                String,
            ),
            ",",
        )
        ...
        demand_downscales = map(
            i -> parse(Float64, i),
            split(readInput("Demand downscales (2): ", "2", String), ","),
        )
    ...
    elseif experiment == "cutoff_error"
        ...
        cutoffs = map(
            ...
            split(
                readInput(
                    "Cutoffs (.0001,.00001,.000001,.0000001): ",
    ...
    elseif experiment == "path_selection"
        ...
        demand_downscales = map(
            i -> parse(Float64, i),
            split(readInput("Demand downscales (4000,4000): ", "4000,4000", String), ","),
        )
        availabilities = map(
            i -> parse(Float64, i),
            split(
                readInput(
                    "Availabilities (.9,.92,.94,.96,.98,.99): ",
                    ".9,.92,.94,.96,.98,.99",
                    String,
                ),
                ",",
    ...
    elseif experiment == "probability_noise"
        ...
        demand_downscales = map(
            i -> parse(Float64, i),
            split(readInput("Demand downscales (1): ", "1", String), ","),
        )
        ...
        noises = map(
            i -> parse(Float64, i),
            split(
                readInput("Noises (.01,.05,.1,.15,.2): ", ".01,.05,.1,.15,.2", String),
                ",",
            ),
        )
        probabilityNoise(
    ...
    elseif experiment == "throughput"
        ...
        demand_downscales = map(
            i -> parse(Float64, i),
            split(readInput("Demand downscales (1,1): ", "1,1", String), ","),
        )
        algorithms = split(
            readInput(
                "Algorithms (TEAVAR,SMORE,ECMP,FFC): ",
                "TEAVAR,SMORE,ECMP,FFC",
                String,
            ),
            ",",
        )
        availabilities = map(
            ...
            split(
                readInput(
                    "Availabilities (.9,.95,.99,.999,.9999): ",
                    ".9,.95,.99,.999,.9999",
                    String,
                ),
                ",",
    ...
    elseif experiment == "utilization"
        ...
        availabilities = map(
            i -> parse(Float64, i),
            split(
                readInput("Availabilities (.99,.999,.9999): ", ".99,.999,.9999", String),
                ",",
            ),
        )
        cutoff = readInput("Cutoff (0.00001): ", 0.00001, Float64)
