...
function teavar(req::HTTP.Request)
    ...
    if topology == "B4"
        ...
        weibull_probs = weibullProbs(length(links), shape = 0.8, scale = 0.001)
    end
    ...
    try
        ...
        if json.path == "ED"
            T, Tf, k, g =
                getTunnels(nodes, links, capacity, flows, 30, edge_disjoint = true)
        else
