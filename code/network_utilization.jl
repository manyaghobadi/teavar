...
function network_utilization(
    ...
    for i = 1:iterations
        link_probs = weibullProbs(length(links), shape = 0.8, scale = 0.0005)
    ...
    progress = ProgressMeter.Progress(
        num_demands * iterations * length(availabilities),
        0.1,
        "Computing Utilization Plot for TEAVAR...",
        50,
    )

    throughputs_teavar = [[] for i = 1:length(availabilities)]
    edge_utilization_percentages = [[] for i = 1:length(availabilities)]
    for b = 1:length(availabilities)
        throughputs = []
        edge_utilizations = []
        for d = 1:num_demands
            avg_throughput = 0
            for i = 1:iterations
                ...
                T, Tf, g =
                    getTunnels(nodes, links, capacity, flows, k, edge_disjoint = false)
                var, cvar, a = TEAVAR(
                    env,
                    links,
                    capacity,
                ...
                edge_utilization, routed = simulateUtilizationNoFailures(
                    links,
                    capacity,
                    demand,
                    flows,
                ...
                edge_utilization_percentages[b] =
                    vcat(edge_utilization_percentages[b], edge_utilization_percentage)
                ProgressMeter.next!(
                    progress,
                    showvalues = [
                        (:beta, availabilities[b]),
                        (:iterations, i),
                        (:num_demands, d),
        ...
        writedlm(
            "$(dir)/$(availabilities[b])_utilization_percentage",
            edge_utilization_percentages,
        )
    end
    ...
    progress = ProgressMeter.Progress(
        num_demands,
        0.1,
        "Computing Utilization Plot for FFC1...",
        50,
    )
    throughputs_ffc1 = []
    ...
    for d = 1:num_demands
        ...
        edge_utilization_percentages_ffc1 =
            vcat(edge_utilization_percentages_ffc1, edge_utilization_percentage)
        ProgressMeter.next!(
            progress,
            showvalues = [(:num_demands, d), (:throughput, throughput)],
        )
    end
    ...
    progress = ProgressMeter.Progress(
        num_demands,
        0.1,
        "Computing Utilization Plot for FFC2...",
        50,
    )
    throughputs_ffc2 = []
    ...
    for d = 1:num_demands
        ...
        edge_utilization_percentages_ffc2 =
            vcat(edge_utilization_percentages_ffc2, edge_utilization_percentage)
        ProgressMeter.next!(
            progress,
            showvalues = [(:num_demands, d), (:throughput, throughput)],
        )
...
function CDF(
    availabilities,
    edge_utilization_percentages_teavar,
    edge_utilization_percentages_ffc1,
    edge_utilization_percentages_ffc2,
    dir,
)
