...
function TEAVAR(
    ...
    # set_optimizer_attribute(model, "OutputFlag", 0) Uncomment this to make Gurobi less verbose
    ...
    for e = 1:nedges
        @constraint(
            model,
            sum(a[f, t] * L[Tf[f][t], e] for f = 1:nflows, t = 1:size(Tf[f], 1)) <=
            capacity[e]
        )
    end
    # end
    ...
    @expression(
        model,
        satisfied[s = 1:nscenarios, f = 1:nflows],
        sum(a[f, t] * X[s, Tf[f][t]] for t = 1:size(Tf[f], 1)) / demand[f]
    )
    ...
    for s = 1:nscenarios
        if average
            @constraint(model, umax[s] + alpha >= (sum(u[s, f] for f = 1:nflows)) / nflows)
    ...
    if length(fixed_flows) > 0
        for f = 1:size(fixed_flows, 1)
            for t = 1:size(fixed_flows, 2)
                println(a[f, t], fixed_flows[f, t])
                @constraint(model, a[f, t] == fixed_flows[f, t])
            end
        end
    end
    ...
    @objective(
        model,
        Min,
        alpha + (1 / (1 - beta)) * sum((p[s] * umax[s] for s = 1:nscenarios))
    )
    optimize!(model)
    ...
    return (value(alpha), objective_value(model), value.(a), value.(u), value.(umax))
end
