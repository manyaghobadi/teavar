...
function SMORE(env, edges, capacity, flows, demand, T, Tf)
    ...
    for f = 1:nflows
        for t = 1:size(Tf[f], 1)
            @constraint(model, sum(a[f, t] for t = 1:size(Tf[f], 1)) == 1)   #the sum of all allocated bandwidths on every flow must be >= the total bandwidth for that flow
        end
    ...
    @expression(
        model,
        U[e = 1:nedges],
        sum(a[f, t] * L[Tf[f][t], e] for f = 1:nflows, t = 1:size(Tf[f], 1)) / capacity[e]
    )
    for e = 1:nedges
