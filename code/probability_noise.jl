...
function probabilityNoise(
    ...
    error_vals = [[] for i = 1:length(noise)]
    ...
    progress = ProgressMeter.Progress(
        length(topologies) * num_demands * iterations * length(noise),
        0.1,
        "Computing Probability Noise Error...",
        50,
    )
        ...
        for d = 1:num_demands
            ...
            for i = 1:iterations
                ...
                scenarios, scenario_probs =
                    subScenarios(weibull_probs, cutoff, first = true, last = false)
                ...
                for j = 1:length(noise)
                    ...
                    if add_noise == "EVENTS"
                        weibull_probs_noise = map(
                            p -> p + (p * noise[j] * rand(Uniform(-1, 1))),
                            weibull_probs,
                        )
                        scenario_probs_noise =
                            getProbabilities(scenarios, weibull_probs_noise)
                        scenario_probs_noise =
                            scenario_probs_noise ./ sum(scenario_probs_noise)
                    else
                        scenario_probs_noise = map(
                            p -> p + (p * noise[j] * rand(Uniform(-1, 1))),
                            scenario_probs,
                        )
                        scenario_probs_noise =
