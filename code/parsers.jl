...
function readDemand(
    ...
    input_demand =
        matrix ? ParseMatrix("$(filename).txt", num_nodes, num_demand) :
        IgnoreCycles(
            readdlm("$(filename)/$(num_demand).txt", header = true)[1],
            zeroindex = zeroindex,
        )
    ...
    for row = 1:size(x, 1)
        if "->" in x[row, :]
            ...
            for col = 2:size(x, 2)
                ...
                e = (
                    parse(Int, stringtup[1]) + zeroindex,
                    parse(Int, stringtup[2]) + zeroindex,
                )
...
function getTunnels(nodes, edges, capacity, flows, k; edge_disjoint = false)
    ...
    for f = 1:num_flows
        ...
        curr_k = 0
        state =
            LightGraphs.yen_k_shortest_paths(graph, flows[f][1], flows[f][2], distances, k)
        paths = state.paths
        edges_used = []
        for i = 1:k
            t = []
