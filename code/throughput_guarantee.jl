...
function getThroughputGuarantees(
    x_vals,
    cutoff,
    topologies,
    num_demands,
    ...
    for t = 1:length(topologies)
        ...
        links, capacity, link_probs, nodes =
            readTopology(topology, downscale = downscale_capacity)
        scenarios_all_top = []
        scenario_probs_top = []
        for i = 1:iterations
    ...
    progress = ProgressMeter.Progress(
        length(x_vals) * iterations * num_demands * length(topologies),
        0.1,
        "Computing TEAVAR...",
        50,
    )
    for b in x_vals
                ...
                for i = 1:iterations
                    ...
                    ProgressMeter.next!(
                        ...
                        showvalues = [
                            ...
                            (:iterations, i),
                            (:num_demands, d),
                            (:satisfied, sum(allowed) / sum(demand)),
                            (:min, minimum(allowed ./ demand)),
                        ],
                    )
                end
            end
        ...
        push!(
            cvar_satisfied,
            sum(beta_satisfied) / (iterations * num_demands * length(topologies)),
        )
    ...
    for t = 1:length(topologies)
        ...
        for d = 1:num_demands
            ...
            ProgressMeter.next!(
                ...
                showvalues = [
                    (:num_demands, d),
                    (:satisfied, sum(b) / sum(demand)),
                    (:min, minimum(b ./ demand)),
                ],
            )
        end
    end
    ...
    writedlm(
        "$(dir)/params",
        [
            ["cutoff", "topologies", "num_demands", "iterations"],
            [cutoff, topologies, num_demands, iterations],
        ],
    )
...
function CDF(num_beta, num)
    ...
    # println(cvar_user)
    cvar_all = [[] for i = 1:num_beta]
    b_num = 0
    for i = 1:size(cvar_user, 1)
        if mod(i, size(cvar_user, 1) / num_beta) == 1
            b_num += 1
        end
        for j = 1:size(cvar_user, 2)
            if cvar_user[i, j] != ""
    ...
    ffc1_all = []
    for i = 1:size(ffc1_user, 1)
        for j = 1:size(ffc1_user, 2)
            if ffc1_user[i, j] != ""
    ...
    ffc2_all = []
    for i = 1:size(ffc2_user, 1)
        for j = 1:size(ffc2_user, 2)
            if ffc2_user[i, j] != ""
