...
function pathSelection(
    ...
    for i = 1:length(topologies)
        ...
        if weibull == true
            weibull_probs = weibullProbs(length(link_probs), shape = 0.8, scale = 0.0001)
            temp_s, temp_p = subScenarios(
                weibull_probs,
    ...
    for p = 1:length(paths)
        ...
        progress = ProgressMeter.Progress(
            length(topologies) * num_demands * length(x_vals),
            0.1,
            "Computing TEAVAR_$(paths[p])...",
            50,
        )
        for i = 1:length(topologies)
            links, capacity, link_probs, nodes = readTopology(topologies[i])
